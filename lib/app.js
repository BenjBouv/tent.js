var requests = require('./requests'),
    utils = require('./utils'),
    makeCredentials = require('./registration');
...
var getApiRoot = function(cb) {
    var that = this;
    this.profileGet( function(err, p) {
        if( err ) {
            cb(err, null);
        } else {
            var core = p[ that.PROFILES.core.url ];
            if( core && core.servers && core.servers.length > 0 ) {
                cb(null, core.servers[0]); // TODO what to do with other servers?
            } else {
...
var register = function(params) {
        ...
        this.getApiRoot(function(err, apiRootUrl) {
            ...
            requests.run({
                ...
                onResult: function(err, headers, data) {
                    ...
                    that.credentials.app = makeCredentials( that.cache.app.mac_algorithm, that.cache.app.mac_key, that.cache.app.mac_key_id );
    ...
    } else if( params.mac_algorithm && params.mac_key && params.mac_key_id ) {

        that.credentials.app = makeCredentials( params.mac_algorithm, params.mac_key, params.mac_key_id );
        if( params.id ) {
...
var get = function( cb ) {
    var that = this;
    if( !that.cache.app || !that.cache.app.id ) {
        cb( 'App id not found', null );
        return;
    }
